...
<?php
    ...
    try {
     $dbh = new pdo( 'mysql:unix_socket=/******/*******:******;dbname=cloudproject',
    'root',
    ''
    );
    if(isset($_POST['login'])) {
        if(isset($_POST['username']) && isset($_POST['password'])){
             ...
             $stmt = $dbh->prepare("select username,password1 from cloudproject.cloud1 where username='$myusername' and password1='$mypassword1'");
             $stmt->execute();
             $row = $stmt->fetchAll(); 
             $count1 = count($row);
             if($count1==1)
